...
export default function Theme(props:ThemeProps) {
  ...
  const ModeIcon = ()=>{
    ...
    switch (theme) {
      case 'light':
        return <Sun size={props.iconSize}></Sun>
      case 'dark':
        return <Moon size={props.iconSize}></Moon>
      case 'system':
        return <Monitor size={props.iconSize}></Monitor>
      default:
        return <Sun size={props.iconSize}></Sun>
    }
  }
  return (
    <div className="relative">
      ...
      {showTheme && <ul className="absolute text-sm duration-500  z-150 top-full left-1/2 -translate-x-1/2  dark:bg-black bg-white border border-zinc-800 rounded-lg ring-1 ring-slate-900/10 shadow-lg overflow-hidden w-36 py-1 text-sm text-slate-700 font-semibold dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-300 mt-8" aria-orientation="vertical" role="listbox">
        ...
        <li onClick={()=>handleSwitchTheme('dark')} data-theme='dark' role="option" className="text-zinc-500 hover:text-zinc-300 py-1 px-2 flex items-center cursor-pointer">
          <Moon size={15}></Moon>
          <span className="ml-4">{themeMap.dark}</span>
        </li>
        <li onClick={()=>handleSwitchTheme('system')} data-theme='system' role="option" className="text-zinc-500 hover:text-zinc-300 py-1 px-2 flex items-center cursor-pointer">
          <Monitor size={15}></Monitor>
